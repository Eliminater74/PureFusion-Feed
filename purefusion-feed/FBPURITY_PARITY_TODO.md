...
- **Last completed (2026-04-14):** Phase 30 — Sponsored Label Detection Hardening. `_isSponsoredLabel`: added `endsWith(' token')`, `endsWith(' · token')`, `includes(' · token ·')`, `includes(' · token')` — catches FB patterns where "Sponsored" is a suffix or mid-string item (e.g. "Page · Sponsored · Follow"). `_findSponsoredMarkerInPost`: height threshold 260→400px (tall posts push label below old limit); text length limit 32→48 chars (allows "Sponsored · 3h ago" combined nodes); element scope expanded to include `[role="link"]` and all `a` elements. `_removeSponsoredByLabels`: added `aria-describedby` scan (multi-ID aware) alongside existing `aria-labelledby`. `sponsoredIndicators`: added `[role="link"][aria-label*=…]` for all 9 locales; added `adabouturl` href; added lowercase `_cft_%5b0%5d` encoding variant. `_removeAdsByHardSignals`: same `adabouturl` + lowercase `_cft_` additions.
