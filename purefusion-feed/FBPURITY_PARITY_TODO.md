...
- **Last completed (2026-04-15):** Phase 32 — Sponsored Toggle-OFF Restoration + Popup Wiring. `_restoreCriticalContainers`: added three reason-string guards before the bail-out check — restores `'Ad (Hard Signal)'` nodes when `removeAds` is OFF; restores `'Sponsored Post (Label Heuristic)'` and legacy `'Sponsored Post (Heuristic)'` nodes when `removeSponsored` is OFF. Fixes DoD regression rule #3 (toggle-OFF must restore content). Browser popup: added secondary `pf-ad-blocker-row` for `tgl_removeSponsored` with 🔍 icon; `popup.js` maps it into `elements.sponsored`, adds it to `quickToggles`, reads initial state from `filters.removeSponsored`, and writes it back in `handleToggle`. i18n: `popup_hide_sponsored_posts` + `_desc` keys added EN+ES.
