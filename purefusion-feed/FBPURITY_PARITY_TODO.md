...
- ✅ Phase 24: Content Script Lifecycle Hardening — COMPLETE (chrome.runtime.connect port guard; destroy() on all modules with running intervals/observers)
- ✅ Phase 25: Memories Filter + Messenger Lifecycle Guard — COMPLETE (removeMemoriesPosts implementation; toggle-OFF reversal; messenger-main lifecycle guard)
- ✅ Phase 26: Notification + Search Popup Full Locale Expansion — COMPLETE (FR/DE/IT/NL/SV/DA/NO phrases for all 4 notification categories + search input selectors + trending/recent heading tokens)
- ✅ Phase 27: UI Tweaks — Unimplemented Settings Activation — COMPLETE (fontSizeScale CSS, anonymizerMode hover-reveal blur, disableCommentAutofocus guard)
...
- **Last completed (2026-04-14):** Phase 27 — UI Tweaks Unimplemented Settings Activation. `fontSizeScale` (80–150): generates `html { font-size: N% }` when not 100; only fires when value is in range and non-default. `anonymizerMode`: CSS blur (8px on imgs, 5px on author name links) with hover-reveal transition; scoped to `[role="article"]`, `[role="complementary"]`, and `[role="navigation"]` to avoid false positives on post text. `disableCommentAutofocus`: capture-phase `focus` listener tracks last `mousedown` target; if a comment textbox gains focus without a matching user click, blurs via microtask to let browser focus machinery complete first. All three settings were wired in options.html/js and default-settings.js but had zero content-script implementation.
- **Prior completed (2026-04-14):** Phase 26 — Notification + Search Popup Full Locale Expansion. Expanded `_filterNotifications` with FR/DE/IT/NL/SV/DA/NO phrase tokens for all 4 categories (games, birthdays, marketplace, engagement suggestions). Expanded `_getHeaderSearchPopupCandidates` with 7 new search input aria-label selectors (FR/DE×2/IT/NL/SV/DA/NO). Expanded `_isLikelySearchPopup` popup-identity tokens to cover all supported locales. Expanded `_filterSearchPopups` trending text list and recent-searches heading list to cover FR/DE/IT/NL/SV/DA/NO. Tokens written in natural form — `_normalizeComparableText` handles diacritic stripping at comparison time.
- **Prior completed (2026-04-14):** Phase 25 — Memories Filter + Messenger Lifecycle Guard. Added `removeMemoriesPosts()` to `cleaner.js`: primary detection via `a[href^="/memories"]` structural signal; secondary via multi-locale text phrases (EN/ES/DE/FR/IT/NL/SV/DA/NO). Safety bailout at 40% threshold. Toggle-OFF reversal added to `_restoreCriticalContainers` — Memories posts are revealed immediately when the setting is disabled. Added `_startLifecycleGuard()` + `_destroy()` to `messenger-main.js` — covers the Messenger surface's MutationObserver (PF_MessengerAI) the same way Phase 24 covered the main content script.
- **Prior completed (2026-04-14):** Phase 24 — Content Script Lifecycle Hardening. Added `_startLifecycleGuard()` to `main.js` — opens a `chrome.runtime.connect` port on boot; `onDisconnect` fires when extension is reloaded/updated and calls `_destroy()`. `_destroy()` stops `PF_Observer`, clears follow-up timers, and calls `destroy()` on `PF_Cleaner` (`_recoveryIntervalId`), `PF_NotificationControls` (`popupScanIntervalId` + `digestIntervalId`), `PF_InPageUI` (`fabDockIntervalId`), `PF_MessengerAI` (MutationObserver). Eliminates stale interval/observer code calling invalidated Chrome APIs after extension reload.
- **Prior completed (2026-04-14):** Phase 23 — Sponsored Detection Locale Completion. Added NL (`Gesponsord`), SV (`Sponsrad`), DA (`Sponsoreret`), NO (`Sponset`) aria-label selectors to `sponsoredIndicators` (both `a` and `span` variants). Added `span` variants for FR/DE/IT which previously only had `a[aria-label]`. Refactored `removeRightRailAds` text scan to drive from `this.sponsoredTokens` — all supported locales now covered automatically without hardcoded strings.
